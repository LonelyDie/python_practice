...
# Проверяем, что начинается с @ и имеет нужную длину
if nickname.startswith("@") and 5 <= len(nickname) <= 15:
    username = nickname[1:]  # Убираем @ для проверки остальных символов
    
    # Проверяем, что ник содержит только буквы и цифры, без заглавных букв
    if username.isalnum() and username == username.lower():
        print("Correct")
    else:
        print("Incorrect")
else:
