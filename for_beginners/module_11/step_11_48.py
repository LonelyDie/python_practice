...
for i in range(n - 1):
    swapped = False  # Флаг, показывающий, произошёл ли обмен элементов на этом проходе
    for j in range(n - i - 1):
        if a[j] > a[j + 1]:
            a[j], a[j + 1] = a[j + 1], a[j]
            swapped = True  # Обмен произошёл
    if not swapped:
        # Если на внутреннем цикле не было обменов, список уже отсортирован
        break
