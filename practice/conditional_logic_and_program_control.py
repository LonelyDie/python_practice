...
for i in range(1, number1 + 1):
    if number1 % i == 0:
        print(f"{i} is a factor of {number1}")
...
# Обрабатываем случай, когда введено не целое число
    ...
    )  # Сообщаем, что введённое значение не является целым числом
...
result1 = cube()  # Вызов функции cube() и сохранение результата (сумма всех бросков)
print(
    result1 / 10_000
...
def coin_flip():
    if random.randint(0, 1) == 0:  # случайный выбор: 0 - "heads", 1 - "tails"
        return "heads"
    else:
        return "tails"
